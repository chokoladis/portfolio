<form action="{{ $route }}" method="post" accept-charset="multipart/form-data">
    <h2>{{ __('Редактирование основной информации') }}</h2>

    @csrf

    <div class="uk-margin">
        <input type="email" name="email" class="uk-input" value="{{ $formData['email'] }}" disabled>
    </div>

    <div class="uk-margin">
        <input type="text" name="fio" require="true" autocomplete="name" placeholder="{{ __('ФИО') }}"
               class="@error('fio') uk-form-danger @enderror uk-input" value="{{ old('fio') ?? $formData['fio'] }}">
